...
5. Cada despliegue usa el `buildCommand` declarado en `vercel.json`: `npm run deploy:build` ejecuta `prisma migrate deploy`, el seed idempotente y después el build. Si una migración falla, Vercel detiene el despliegue.
