...
1. Vincula Neon desde Vercel. La integración inyecta `DATABASE_URL` para ejecución y normalmente `DATABASE_URL_UNPOOLED` para migraciones; ambas deben habilitarse en Preview y Production cuando estén disponibles.
...
5. Cada despliegue usa el `buildCommand` declarado en `vercel.json`: `npm run build` ejecuta `scripts/deploy-build.mjs`. El script prefiere `DATABASE_URL_UNPOOLED`, admite `DIRECT_URL` como compatibilidad y usa `DATABASE_URL` como respaldo; después aplica `prisma migrate deploy`, ejecuta el seed idempotente y compila Next.js. Si una migración falla, Vercel detiene el despliegue.
6. Configura `SUPERADMIN_EMAIL` y `SUPERADMIN_PASSWORD` (mínimo 16 caracteres). Al iniciar sesión por primera vez, NEXO crea o actualiza esa cuenta como `SUPERADMIN` y la envía a `/admin`. La cuenta sólo puede autenticarse contra esas variables.
