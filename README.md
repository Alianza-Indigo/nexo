...
Vercel toma el comando de despliegue del propio repositorio. El comando estándar `npm run build` selecciona la conexión directa de Neon cuando existe, ejecuta todas las migraciones pendientes con `prisma migrate deploy`, carga el seed idempotente y compila Next.js; GitHub Actions comprueba la cadena completa contra PostgreSQL vacío.
