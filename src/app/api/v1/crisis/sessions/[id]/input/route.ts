...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const turn = processTurn({ state: session.currentState as Parameters<typeof processTurn>[0]["state"], answer: body.modality === "button" ? body.input : undefined, text: body.input, context, transcriptUncertain: body.transcriptUncertain });
    ...
    await db.$transaction(async (tx) => {
      ...
      await tx.safetyAssessment.create({ data: { crisisSessionId: id, messageId: userMessage.id, flags: turn.risk.flags, riskLevel: turn.risk.level, uncertainty: turn.risk.uncertain, evidenceCategories: turn.risk.evidenceCategories, rulesVersion: "1.0" } });
