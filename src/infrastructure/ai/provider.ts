...
export const aiTurnSchema = z.object({
  ...
  observations: z.object({ age_band: z.string().nullable(), environment: z.string().nullable(), driving: z.boolean().nullable(), behavior: z.string().max(500).nullable(), preceding_event: z.string().max(500).nullable(), known_support: z.string().max(300).nullable() }),
...
export function createAIProvider(name = process.env.AI_PRIMARY_PROVIDER ?? "deterministic", key = process.env.AI_PRIMARY_API_KEY ?? ""): AIProvider { if (name === "openai" && key) return new OpenAIProvider(process.env.AI_PRIMARY_MODEL ?? "gpt-5-mini", key); if (name === "gemini" && key) return new GeminiProvider(process.env.AI_PRIMARY_MODEL ?? "gemini-2.5-flash", key); if (name === "anthropic" && key) return new AnthropicProvider(process.env.AI_PRIMARY_MODEL ?? "claude-sonnet-4-5", key); return new DeterministicProvider(); }
